# step 1: open python script; then open Max patch; then run script; then start process in patch
# reads a file called movie_paths.txt and loads all contained paths (which are automatically added from the max patch)
...
<<<<<<< HEAD
 # directories is overwritten before it's used (by the movie_paths text file)
 layers = 16
=======
 layers = LAYERS
>>>>>>> 8a0cd948342c048f776d4ab18c679b3a37aec399
...
# compile list of videos
 ...
 while x < layers:
    ...
    while 1: # this is a loop to allow the programme to loop (creating multiple videos); it is cancelled by a break at the end signal 
        ...
        print(chosen_video, inputs[chosen_video])
